...
struct Shape * quickSort(struct Shape *shapes, int sizeToSort) {
    float pivotPoint = selectPivot(shapes, sizeToSort);
}
/*
 ...
 * lomutos median of 3 of the middle lo and high values for each partition.
 * When using 
 */
int selectPivot(struct Shape *shapes, int size) {
    int median[3] = {0,0,0};
    int remainder = size%3;//how many left over elements that won't be handled by the loop
    ...
    if(remainder==0){
        ...
        return medianOfThree(median[0], size-1 size-2);
    }
    else{
        return medianOfThree(median[0], size-1, rand()%(size-1));
    }       
}
